 ...
 *  添加img的编辑
 */
angular.module('xeditable').directive('editableImg', ['editableDirectiveFactory', 'httpService', '$q',
    function (editableDirectiveFactory, httpService, $q) {
        return editableDirectiveFactory({
            directiveName: 'editableImg',
            useCopy: false,
            inputTpl: '<input type="hidden">',
            render: function () {
                this.parent.render.call(this);
                var self = this;
                self.scope.defaultSrc = this.attrs.eTableNgfDefaultSrc;
                var progress = angular.element("<div ng-if=\"progress\" class=\"dz-progress\"><div class=\"img-progress\">{{uploadProgressPercentage}}%<\/div><\/div>")

                var img = angular.element("<img class=\"img-responsive\" ngf-src=\"files[0]\" ngf-default-src=\"{{defaultSrc}}\">");
                img.attr("style", this.attrs.eStyle);

                var btn = angular.element("<div>" +
                    "<button accept=\"image\/*\" class=\"button btn btn-default\" ngf-select ng-model=\"files\" ngf-multiple=\"false\">选择图片 <\/button>" +
                    "<button type=\"button\" class=\"button btn btn-default\" ng-click=\"upload(files)\">提交<\/button>" +
                    "<button type=\"button\" class=\"button btn btn-default\" ng-click=\"delete(files)\">删除<\/button>" +
                    "<\/div>")

                this.inputEl.wrap("<div class='relative'><\/div>");
                //this.inputEl.after(image);
                this.inputEl.after(btn);
                this.inputEl.after(img);
                this.inputEl.after(progress);
                ...
                self.scope.delete = function () {
                    self.scope.files = null;
                    img.attr("src", '');
                    self.scope.defaultSrc = null;
                    self.scope.$data = null;
                }

                //self.onbeforesave = function () {
                //    var files = self.scope.files;
                //    var d = $q.defer();
                //    console.log(self.scope.$data, self.scope.files);
                //    if (!files) {
                //        d.resolve({});
                //    } else {
                //        positionPopover();
                //        self.scope.progress = true;
                //        httpService.update({
                //            file: files[0],
                //            progress: function (evt) {
                //                self.scope.uploadProgressPercentage = parseInt(100.0 * evt.loaded / evt.total);
                //                console.log('progress: ' + self.scope.uploadProgressPercentage + '% ' + evt.config.file.name);
                //            },
                //            success: function (response, status, headers, config) {
                //                if (response.result == "success") {
                //                    httpService.post({
                //                        url: 'reference/imageGrid',
                //                        data: {
                //                            "fieldName": "",
                //                            "fieldSeriNum": "",
                //                            "description": "",
                //                            "imageRepo": {"id": response.data.id}
                //                        },
                //                        success: function (response_) {
                //                            self.scope.progress = false;
                //                            if (response_.result == "success") {
                //                                self.scope.$data = response_.data
                //                                d.resolve(response);
                //                            }
                //                        },
                //                        error: function () {
                ...
                self.scope.upload = function (files) {
                    if (!files || !files[0]) {
                        return;
                    }
                    positionPopover();
                            ...
                            if (response.result == "success") {
                                httpService.post({
                                    ...
                                    success: function (response_) {
                                        ...
                                        if (response_.result == "success") {
                                            self.scope.$data = response_.data
                                            self.scope.defaultSrc = self.scope.$data.imageRepo.pathOrigProp;
                                        }
                                    },
                                    error: function () {
                                        self.scope.progress = false;
 ...
 *  添加多张img的编辑
 */
//angular.module('xeditable').directive('editableImgs', ['editableDirectiveFactory', 'httpService',
//        function (editableDirectiveFactory, httpService) {
//
//        }]
//)
/** 修正
 *  Angular-ui bootstrap datepicker 直接应用
 */
angular.module('xeditable').directive('editableMyDate', ['editableDirectiveFactory',
    function (editableDirectiveFactory) {
        return editableDirectiveFactory({
            directiveName: 'editableMyDate',
            inputTpl: '<input style="border-right: medium none;" type="text"/>',
            controlsTpl: '<div class="editable-controls input-group customer-group" ng-class="{\'has-error\': $error}"></div>'
        });
    }]);
/** 修正
 *  Angular-ui bootstrap BtnCheckbox
 */
angular.module('xeditable').directive('editableBtnCheckbox', ['editableDirectiveFactory',
    function (editableDirectiveFactory) {
        return editableDirectiveFactory({
            directiveName: 'editableBtnCheckbox',
            inputTpl: '<button type="button" ng-model="$data" btn-checkbox btn-checkbox-true="1" btn-checkbox-false="0"></button>',
            render: function () {
                this.parent.render.call(this);
                this.inputEl.append('<input ng-checked="$data" type="checkbox">');
            }
        });
    }]);
///** 修正
        ...
        function ($q, editableUtils) {
            ...
            function EditableController($scope, $attrs, $element, $parse, editableThemes, editableIcons, editableOptions, $rootScope, $compile, $q) {
                ...
                self.render = function () {
                    ...
                    //build controls
                    if (self.controlsTpl) {
                        self.controlsEl = angular.element(self.controlsTpl);
                    } else {
                    ...
                    //if($attrs.$attr["eRequired"]){
                    //    self.inputEl.after("<span>*<\/span>")
                    //}
                ...
                self.hide = function () {
                     ...
                     * 隐藏的时候移除校验的类
                     */
                    var formGroupEl = findWithClassElementAsc($element, 'form-group')
                    if (formGroupEl) {
                        formGroupEl.removeClass('has-success has-error has-feedback');
                    }
                    // onhide
                    return self.onhide();
                };
